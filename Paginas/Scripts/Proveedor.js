...
async function Consultar() {
    ...
    try {
        ...
        const Resultado = await fetch("http://localhost:55644/api/Producto/ConsultarXCodigo?Codigo=" + Documento,
            {
                method: "GET",
                mode: "cors",
        ...
        $("#txtRazonSocial").val(Respuesta.razon_social);
...
function LlenarTabla() {
    LlenarTablaXServicios('http://localhost:55644/api/Proveedores/ListarConTipoDocumento', "#tblProveedores")
}

async function EjecutarComando(Metodo, Funcion) {
    const cliente = new Cliente($("#txtNombre").val(), $("cboTipoDocumento").val(), $("#txtDocumento").val());
    try {
        const Resultado = await fetch("http://localhost:55644/api/Proveedores/api/Proveedores/" + Funcion,
            {
                method: Metodo,
                mode: "cors",
                headers: { "Content-Type": "application/json" },
...
class Proveedor {
    constructor(idTipoDocumento, NroDocumento,RazonSocial, Activo) {
